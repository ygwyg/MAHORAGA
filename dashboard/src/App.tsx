...
export default function App() {
  ...
  const { marketMarkers, marketHoursZone } = useMemo(() => {
    ...
    portfolioHistory.forEach((s, i) => {
      ...
      } else if (etHours === 16 && etMinutes === 0 && closeIndex === -1) {
        closeIndex = i;
          ...
          <div className="text-center py-8">
            ...
            {isAuthError ? (
              <div className="space-y-4">
                ...
                >
                  <label htmlFor="api-token" className="hud-label block mb-2">
                    API Token
                  </label>
                  <input
                    id="api-token"
                ...
                <p className="text-hud-text-dim text-xs">
                  Find your token in <code className="text-hud-primary">.dev.vars</code> (local) or Cloudflare secrets
                  (deployed)
                </p>
              </div>
            ) : (
              <p className="text-hud-text-dim text-xs">
                Enable the agent:{" "}
                <code className="text-hud-primary">
                  curl -H "Authorization: Bearer $TOKEN" localhost:8787/agent/enable
                </code>
              </p>
            )}
          </div>
                              ...
                              <div className="pt-1 border-t border-hud-line/30">
                                ...
                                <ul className="mt-1 space-y-0.5">
                                  {research.catalysts.map((c, i) => (
                                    <li key={i} className="text-[10px] text-hud-success">
                                      + {c}
                                    </li>
                                  ))}
                                </ul>
                              ...
                              <div className="pt-1 border-t border-hud-line/30">
                                ...
                                <ul className="mt-1 space-y-0.5">
                                  {research.red_flags.map((f, i) => (
                                    <li key={i} className="text-[10px] text-hud-error">
                                      - {f}
                                    </li>
                                  ))}
                                </ul>
